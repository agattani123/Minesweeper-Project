#!/usr/bin/env bash
# install-claude-logger.sh — installs Claude prompt logging with continuous monitoring
set -euo pipefail
...
if grep -q "claude() {" ~/.zshrc 2>/dev/null; then
  ...
  # Use a more precise removal that handles the full function
  awk '
  /^claude\(\) \{/ { in_func=1; next }
  in_func && /^}$/ { in_func=0; next }
  !in_func { print }
  ' ~/.zshrc > ~/.zshrc.tmp && mv ~/.zshrc.tmp ~/.zshrc
fi

# Add new claude function with continuous monitoring and exact VS Code extension behavior
cat >> ~/.zshrc << 'EOF'
claude() {
  if git rev-parse --show-toplevel >/dev/null 2>&1; then
  ...
  process_new_prompts() {
    ...
    while IFS= read -r line; do
      ...
      local clean_line=$(echo "$line" | LC_ALL=C sed -E 's/\x1b\[[0-9;]*[a-zA-Z]//g' | tr -d '\r' | sed 's/^[[:space:]]*//;s/[[:space:]]*$//')
...
echo "✔ Configured Claude with continuous monitoring (processes every 5 seconds)"
echo "   Log file: $REPO_ROOT/prompts.txt"
echo "   Features:"
echo "     • Continuous prompt monitoring every 5 seconds while Claude runs"
echo "     • Exact VS Code extension parsing logic"
echo "     • Deduplication to prevent duplicate logging"
echo "     • ANSI code cleaning and proper filtering"
...
echo ""
echo "🎉 Installation complete! Your Claude prompts will now be logged to prompts.txt"
echo "   in any git repository, with real-time monitoring every 5 seconds."